...
if __name__ == '__main__':
    ...
    parser.add_argument("input_dir", help="path to directory for input file list")
    parser.add_argument("output_dir", help="path to directory for output file list")
    parser.add_argument("-p", "--pattern", help="glob pattern for input files", default='*.xml')
    parser.add_argument("-o", "--outpath", help="directory where filelists should be written")
    args = parser.parse_args()

    inpath = os.path.join(args.input_dir, args.pattern)
    ins = []
    outs = []
    for i in glob(inpath):
        ins.append(i)
        fname, _ = os.path.splitext(os.path.basename(i))
        outs.append(os.path.join(args.output_dir, fname + '.conllu'))
